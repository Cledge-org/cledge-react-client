import { getAllQuestionLists } from "../src/pages/api/questions/get-all-questions";
import { getQuestionList } from "../src/pages/api/get-question-list";
import { getQuestionProgress } from "../src/pages/api/get-question-progress";
import { getQuestionResponses } from "../src/pages/api/get-question-responses";
import { putQuestionChunk } from "../src/pages/api/admin/question/put-question-chunk";
import { putQuestionList } from "../src/pages/api/put-question-list";
import { putQuestionResponses } from "../src/pages/api/put-question-responses";
import { putQuestion } from "../src/pages/api/put-question";
...
test("should add one question and get that one added question exactly", (done) => {
  async function callback() {
    ...
    expect(fetchedQuestionsProgress.questionData[0]).toMatchObject(
      testQuestionList1
    );
...
test("should verify many questions", (done) => {
  function createQuestion(
    ...
    questionListDb: QuestionList_Db;
    questionChunkDb: QuestionChunk_Db;
  } {
    ...
    return {
      ...
      questionChunk: questionChunk,
      questionList: questionList,
      questionListDb: questionListDb,
      questionChunkDb: questionChunkDb,
    };
  }
  ...
  async function callback() {
    ...
    for (let i = 0; i < manySizes; i++) {
      ...
      await putQuestionList(
        questionListObjectId,
        questionResult.questionListDb
      );
      await putQuestionChunk(
        questionChunkObjectId,
        questionResult.questionChunkDb
      );
      await putQuestion(questionId, questionResult.question);
      await putQuestionResponses(userId, [questionResult.userResponse]);
    }

    const [fetchedAllQuestionsLists, fetchedQuestionsProgress] =
      await Promise.all([getAllQuestionLists(), getQuestionProgress()]);
    ...
    for (let i = 0; i < manySizes; i++) {
      ...
      expect(fetchedQuestionList.chunks[0]).toMatchObject(
