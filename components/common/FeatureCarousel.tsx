...
import { faEdit, faChalkboardTeacher, faComments, faSearch } from "@fortawesome/free-solid-svg-icons";
...
export default () => {
    ...
    return (
        <div style={{backgroundColor:'#E5E5E5'}} className='pt-0 pt-md-4' >
            <div className='row'>
                ...
                <div className='row p-3 justify-content-evenly'>
                    <div className='col-6 col-lg-3'>
                        <FeatureTab index={0} curr={index}  title={'Personalization Quizes'} icon={faEdit} onClick={()=>handleOnClick(0)}/>
                    </div>
                    <div className='col-6 col-lg-3'>
                        <FeatureTab index={1} curr={index}    title={'Video Learning Pathway'} icon={faChalkboardTeacher} onClick={()=>handleOnClick(1)}/>
                    </div>
                    <div className='col-6 col-lg-3'>
                        <FeatureTab index={2} curr={index}    title={'AI Counselor'} icon={faComments} onClick={()=>handleOnClick(2)}/>
                    </div>
                    <div className='col-6 col-lg-3'>
                        <FeatureTab index={3} curr={index}    title={'College Search Tool'} icon={faSearch} onClick={()=>handleOnClick(3)}/>
                    </div>
                </div>
                </div>
            </div>
...
const FeatureTab = ({icon, title, onClick, curr, index}: FeatureTabProps) => {
    ...
    return (
        <div className='feature-tab d-flex flex-column justify-content-between' onClick={(e)=>onClick()} style={{borderColor: borderColor}}>
            <FontAwesomeIcon icon={icon} style={{height:'2.4em'}} className="cl-blue"/>
            <p className="pt-3 m-0 cl-darktext text-nowrap overflow-hidden" style={{fontSize:'0.9em'}}>{title}</p>
        </div>
    )
}