...
import getQuestionMappings, {
  MappedValues,
  UnMappedValues,
} from "../../../../ClientSideFunctions/getQuestionMappings";
...
const FormCarousel = ({ questionData, collegeData }: Props) => {
  ...
  questionData.map((data) => (initialFormData[data._id] = data.data[0].tag));
  const [currentSlide, setCurrentSlide] = React.useState(0);
  const [formData, setFormData] =
    React.useState<UnMappedValues>(initialFormData);
  ...
  const handleSubmit = async () => {
    let preferences = addPreferenceLevel(
      getQuestionMappings(formData, CheckinQuestions)
    );
  ...
  return (
    <div className={cs(styles.carouselContainer)}>
      <div className={cs(styles.carousel,currentSlide > 0 && currentSlide < numSlides+1 && styles.hasDots)}>
        <form
          onSubmit={(e) =>
      ...
      {currentSlide > 0 && currentSlide <= numSlides && <Dots>
        <button
          disabled={currentSlide === 1 || currentSlide === numSlides+1}
        ...
        {[...new Array(numSlides)].map((d, _i) => (
          <Dot
            key={_i}
            onClick={() =>
              currentSlide !== numSlides + 1 && setCurrentSlide(_i+1)
            }
            className={cs(
              styles.dot,
              _i === currentSlide-1 ? "bg-cl-orange" : "bg-cl-white"
            )}
          />
        ))}
        <button
