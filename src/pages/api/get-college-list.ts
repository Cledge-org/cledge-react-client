...
export default async (req: NextApiRequest, resolve: NextApiResponse) => {
    ...
    if (user_id) {
        try {
            ...
            resolve.status(200).send({"status": "Success", "college_list": collegeListInfo});
        } catch (e) {
            console.log(e);
...
export const getCollegeIdList = (user_id: String): Promise<Object> => {
    return new Promise(async(res, err) => {
        try {
            ...
            const userCollegeList = await userDb.collection("college-list").findOne({ firebaseId: user_id });
            if (userCollegeList) {
...
export const getCollegeListInfo = async (
    ...
    return new Promise(async(res, err) => {
        try {
            ...
            const collegeListData = await collegeDb.collection("colleges").find({"college_id": {"$in": collegeIdList}}).toArray();
...
const formatCollegeListInfoOutput = (collegeList, collegeListData) => {
    ...
    const output = [];
    collegeList.forEach((curCollege) => {
        const curCollegeInfo = Object.assign(curCollege, collegeListDataDict[curCollege.college_id]);
